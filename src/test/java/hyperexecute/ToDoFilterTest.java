...
public class ToDoFilterTest {
    ...
    public void test_ToDoFilters(String browserName, String version, String platform, String build, String name) {
        ...
        try {
            ...
            driver.findElement(By.cssSelector("button[type='submit']")).click();
            Thread.sleep(1000);
            System.out.println("Added Task A and Task B");

            // Mark Task A as done
            driver.findElement(By.xpath(
                    "//li[.//span[text()='Task A']]//input[@type='checkbox']")).click();
            Thread.sleep(1000);
            System.out.println("Marked Task A as done");

            // Click Active filter
            driver.findElement(By.xpath(
                    "//button[contains(@class,'filter-btn') and .//span[text()='Active']]")).click();
            Thread.sleep(1000);

            // Verify Task B visible, Task A hidden
            wait.until(ExpectedConditions.visibilityOfElementLocated(
                    By.xpath("//span[@class='todo-text'][text()='Task B']")));
            driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(1));
            assertTrue(driver.findElements(
                    By.xpath("//span[@class='todo-text'][text()='Task A']")).isEmpty(),
                    "Task A should not be visible in Active filter");
            driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(10));
            System.out.println("Active filter verified");
            ...
            // Verify Task A visible, Task B hidden
            wait.until(ExpectedConditions.visibilityOfElementLocated(
                    By.xpath("//span[@class='todo-text'][text()='Task A']")));
            driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(1));
            assertTrue(driver.findElements(
                    By.xpath("//span[@class='todo-text'][text()='Task B']")).isEmpty(),
                    "Task B should not be visible in Completed filter");
            driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(10));

            // Verify Clear completed button visible
            assertTrue(driver.findElement(By.xpath(
                    "//button[contains(text(),'Clear completed')]")).isDisplayed(),
                    "Clear completed button should be visible");
            System.out.println("Completed filter verified");

            // Click All filter and verify both items visible
            driver.findElement(By.xpath(
                    "//button[contains(@class,'filter-btn') and .//span[text()='All']]")).click();
            Thread.sleep(1000);
            wait.until(ExpectedConditions.visibilityOfElementLocated(
                    By.xpath("//span[@class='todo-text'][text()='Task A']")));
            wait.until(ExpectedConditions.visibilityOfElementLocated(
                    By.xpath("//span[@class='todo-text'][text()='Task B']")));
            System.out.println("All filter verified");

            System.out.println("ToDo Filter test completed successfully");
            status = "passed";
        } catch (Exception e) {
            status = "failed";
