...
public class ToDoAddAndComplete5Test {
        ...
        public void test_ToDoApp5(String browserName, String version, String platform, String build, String name) {
                ...
                try {
                        ...
                        WebElement textInput = driver.findElement(
                                        By.cssSelector("input[placeholder='What needs to be done?']"));
                        textInput.sendKeys("Yey, Let's add it to list");
                        driver.findElement(By.cssSelector("button[type='submit']")).click();
                        Thread.sleep(1000);
                        System.out.println("Added first item to list");

                        // Verify first item appears
                        WebElement addedItem = wait.until(ExpectedConditions.visibilityOfElementLocated(
                                        By.xpath("//span[@class='todo-text'][text()=\"Yey, Let's add it to list\"]")));
                        assertTrue(addedItem.isDisplayed(), "Added item should be visible");

                        // Add second todo item
                        textInput = driver.findElement(
                                        By.cssSelector("input[placeholder='What needs to be done?']"));
                        textInput.sendKeys("Complete HyperExecute testing");
                        driver.findElement(By.cssSelector("button[type='submit']")).click();
                        Thread.sleep(1000);
                        System.out.println("Added second item to list");

                        // Mark first item as done
                        WebElement firstItemCheckbox = driver.findElement(By.xpath(
                                        "//li[.//span[text()=\"Yey, Let's add it to list\"]]//input[@type='checkbox']"));
                        firstItemCheckbox.click();
                        Thread.sleep(1000);
                        System.out.println("Marked first item as done");

                        // Verify first item has 'done' class
                        WebElement firstItem = driver.findElement(By.xpath(
                                        "//li[.//span[text()=\"Yey, Let's add it to list\"]]"));
                        assertTrue(firstItem.getAttribute("class").contains("done"),
                                        "Completed item should have 'done' class");

                        // Verify counters
                        WebElement stats = driver.findElement(By.cssSelector(".topbar-stats"));
                        assertTrue(stats.getText().contains("1") && stats.getText().contains("remaining"),
